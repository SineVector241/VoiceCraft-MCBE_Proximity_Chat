...
namespace VoiceCraft.Client.PDK.Audio
{
    ...
    {
        public static int FilterLengthMS = 100; //100ms filter length, can be changed by plugins using reflection or casting.
        ...
        public void Init(IAudioRecorder recorder, IAudioPlayer player)
        {
            ...
            try
            {
                ...
                _echoCanceler.Ctl<int>(SpeexDSPSharp.Core.EchoCancellationCtl.SPEEX_ECHO_SET_SAMPLING_RATE, ref sampleRate);
            }
            catch (Exception ex)
            {
